...
<body class="flex flex-col items-center justify-between min-h-screen bg-gray-200 text-center">
    <div class="flex flex-col items-center justify-center w-full h-1/3">
        <h1 class="text-black font-bold p-6 text-7xl my-4 lg:text-6xl">Smart Silvia</h1>
        <div id="statusMessage" class="text-4xl lg:text-2xl my-2">
            <span id="statusDot" class="status-dot green-dot"></span>
            <span id="statusText">Online</span>
        </div>
        <div id="currentTemp" class="text-6xl lg:text-4xl my-2">
            Current Temperature: <span id="temperature">28.88</span>°C
            <span id="tempStatusDot" class="status-dot orange-dot"></span>
        </div>
        <div id="targetTemp" class="text-6xl lg:text-4xl my-2">
            Target Temperature: <span id="targetTemperature">98</span>°C
        </div>
        <div id="tempSetContainer" class="flex flex-col items-center mt-3 mb-3">
            <input type="number" id="tempInput" min="90" max="98" placeholder="Target temp" required class="mb-4 p-3 text-5xl lg:text-3xl border border-gray-300 rounded text-center w-full max-w-xs">
            <button onclick="setTemperature()" class="py-3 px-5 bg-gray-300 text-black rounded hover:bg-gray-400 text-5xl lg:text-3xl">Set Temperature</button>
        </div>
    </div>
